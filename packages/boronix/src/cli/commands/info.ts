...
export async function infoCommand(
  ...
  // 3. Gather Boronix Info
  const version = getBoronixVersion()
  ...
  if (isPlain) {
    console.log(`* Boronix info`)
  } else {
    console.log(`${colors.brand(symbols.header())} ${colors.bold("Boronix info")}`)
  }
  console.log("")
...
function getBoronixVersion(): string {
  try {
