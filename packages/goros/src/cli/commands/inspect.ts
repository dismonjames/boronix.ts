...
export async function inspectCommand(
  ...
  if (!matched) {
    throw new GorosUserError(`No route matched \`${routePath}\`.`, {
      code: "KQ_ROUTE_NOT_FOUND",
      hint: "Run `goros routes` to see available routes."
    })
  }
  ...
  // Print Header
  if (isPlain) {
    console.log(`* Goros inspect`)
  } else {
    console.log(`${colors.brand(symbols.header())} ${colors.bold("Goros inspect")}`)
  }
  console.log("")
