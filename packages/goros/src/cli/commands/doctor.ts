...
export async function doctorCommand(
  ...
  // 2. goros.config.ts found
  const configExists = existsSync(path.join(root, "goros.config.ts"))
  projectChecks.push({
    label: configExists ? "goros.config.ts found" : "goros.config.ts missing",
    status: configExists ? "success" : "error",
    hint: !configExists ? "Create a `goros.config.ts` configuration file." : undefined
  })
  ...
  if (config) {
    ...
    const isSecretDefault = config.session.secret === "goros-dev-session-secret"
  ...
  // Print results
  if (isPlain) {
    console.log(`* Goros doctor`)
  } else {
    console.log(`${colors.brand(symbols.header())} ${colors.bold("Goros doctor")}`)
  }
  console.log("")
