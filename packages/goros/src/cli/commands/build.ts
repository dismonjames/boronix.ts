...
type BuildRouteEntry = {
  ...
  success: boolean
  error?: GorosUserError | undefined
}
...
export async function buildCommand(
  ...
  if (isPlain) {
    ...
    console.log(`  ${colors.brand(symbols.output())} ${colors.muted("output").padEnd(9)} ${colors.bold(".goros")}`)
    console.log("")
  }

  // Compile and validate entries
  const entries: BuildRouteEntry[] = []
  let firstError: GorosUserError | undefined = undefined
  ...
  for (const item of pageItems) {
    ...
    let success = true
    let error: GorosUserError | undefined = undefined
    ...
    if (item.pageModule && existsSync(item.pageModule)) {
      ...
      if (!content.includes("export default")) {
        success = false
        error = new GorosUserError("Invalid page loader.", {
    ...
    if (item.actionsModule && existsSync(item.actionsModule)) {
      ...
      let actSuccess = true
      let actError: GorosUserError | undefined = undefined

      const content = readFileSync(item.actionsModule, "utf8")
      if (content.includes("export default")) {
        actSuccess = false
        actError = new GorosUserError("Invalid default export in actions module. Actions must be named exports.", {
