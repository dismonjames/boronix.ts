...
async function run() {
  console.log("\x1b[38;5;208m◆\x1b[0m \x1b[1mcreate-goros\x1b[0m\n")
  ...
  if (isInteractive) {
    ...
    try {
      // 1. Project name
      const nameAns = await rl.question("Project name: (my-goros-app): ")
      projectName = nameAns.trim() || "my-goros-app"
    ...
    if (!projectName) {
      console.error("\x1b[31m✖\x1b[0m \x1b[1mUsage:\x1b[0m create-goros <app-name> [options]")
      process.exit(1)
    }
  ...
  if (existsSync(pkgPath)) {
    ...
    // Set goros version to ^0.2.5
    if (pkg.dependencies && pkg.dependencies.goros) {
      pkg.dependencies.goros = "^0.2.5"
    }
  ...
  const configPath = path.join(targetDir, "goros.config.ts")
